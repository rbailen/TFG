        ...
        <script type="text/javascript">
            $(function () {
                $('#myModal').on('show.bs.modal', function (event) {
                    ...
                    var idEntrada = button.data('id');
                    var modal = $(this);
                    modal.find('#btEliminar').attr('href', '<c:url value="/entradas/borraEntrada/"/>' + idEntrada);
                });
            });
        </script>
    </head>
            ...
            <div class="row">
                ...
                <div class="col-md-10">
                    ...
                    <c:if test="${empty entradas}">
                        <center>NO HAY ENTRADAS DISPONIBLES</center>
                        </c:if>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <c:forEach var="c" items="${entradas}">
                                    <c:set var="qry" value="${c.idEntrada}"/>
                                    <tr class="table table-striped">
                                        <td>${c.idEntrada}</td>
                                        <td>${c.descripcion}</td>
                                        <td>${c.precio} €</td>
                                        <td>${c.evento.nombre}</td>
                                        <td>${c.comprada?"Sí":"No"}</td>

                                        <td>
                                            
                                            <c:set var="alaventa" value="false"/> 
                                            <c:forEach var="entrada" items="${entradasCompradas}">
                                                <c:if test="${entrada.idEntrada == c.idEntrada}">
                                                    <c:set var="alaventa" value="true"/> 
                                                    Entrada vendida
                                                </c:if>
                                                
                                            </c:forEach>
                                            
                                            <c:if test="${not alaventa}">
                                                <a href="<c:url value='/entradas/editaEntrada/${qry}'/>" class="btn btn-primary">
                                        ...
                                        <td>
                                            <a class="btn btn-danger" role="button" data-id="${c.idEntrada}" data-toggle="modal" data-target="#myModal"><span class="glyphicon glyphicon-trash"></span>&nbsp;Eliminar</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </table>
                    ...
                    <a href="<c:url value='/entradas/altaEntrada'/>" class="btn btn-success">
                        <span class="glyphicon glyphicon-plus"></span>&nbsp;Nueva Entrada
                    </a>
                    <c:if test="${empty eventos}">
                        <a href="<c:url value='/entradas/venta'/>" class="btn btn-warning" disabled="disabled">
                            <span class="glyphicon glyphicon-user"></span>&nbsp;Puesta en Venta
                        </a>
                    </c:if> 
                    <c:if test="${not empty eventos}">
                        <a href="<c:url value='/entradas/venta'/>" class="btn btn-warning">
                            <span class="glyphicon glyphicon-user"></span>&nbsp;Puesta en Venta
                        </a>
                    </c:if>
                </div>
